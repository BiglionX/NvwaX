...
export default function TeamExecutionPage() {
  ...
  const handleExecute = async () => {
    if (!requirement.trim()) {
      setError(t('errorTaskRequirement'));
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t('errorExecutionFailed'));
      }
    } catch (err) {
      console.error('Execution failed:', err);
      const errorMessage = err instanceof Error ? err.message : t('errorRetry');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-linear-to-br from-gray-50 via-blue-50/30 to-blue-50/30 dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 min-h-screen">
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 rounded-xl p-4 mb-6 flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-red-800 dark:text-red-300 mb-1">{t('executionFailed')}</h3>
          ...
          <div className="space-y-4">
            ...
            {executionResult.teammates && executionResult.teammates.length > 0 && (
              <div>
                <h3 className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  {t('teamMembers', { count: executionResult.teammates.length })}
                </h3>
            ...
            {executionResult.workflowSteps && (
              <div>
                ...
                <p className="text-gray-900 dark:text-white">{t('workflowSteps', { count: executionResult.workflowSteps })}</p>
              </div>
            )}
            ...
            {executionResult.executionTime && (
              <div>
                ...
                <p className="text-gray-900 dark:text-white">
                  {t('executionTimeValue', { seconds: (executionResult.executionTime / 1000).toFixed(2) })}
                </p>
              </div>
            )}
