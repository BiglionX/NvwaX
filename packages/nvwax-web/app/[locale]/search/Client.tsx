  ...
  const renderSkillCard = (skill: Skill) => (
    <div key={skill.id} className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow border border-gray-200 dark:border-gray-700">
      ...
      {skill.usageCount !== undefined && (
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">{t('usageCount')}: {skill.usageCount}</p>
      )}
      <button className="text-blue-600 dark:text-blue-400 hover:underline text-sm font-medium">{t('addToProject')}</button>
    </div>
  );
          ...
          <div className="mt-4 p-6 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {activeTab === 'agents' && (
                <>
                  {loadingAgents ? (
                    <div className="col-span-full text-center py-12 text-gray-500">{t('loading')}</div>
                    ...
                    <div className="col-span-full text-center py-12 text-gray-500">{t('noQuery')}</div>
                  )}
                </>
              )}
              {activeTab === 'skills' && (
                <>
                  {loadingSkills ? (
                    ...
                    <div className="col-span-full text-center py-12 text-gray-500">{t('noResultsSkills')}</div>
                  ) : (
                    <div className="col-span-full text-center py-12 text-gray-500">{t('noQuery')}</div>
                  )}
                </>
            ...
            {((activeTab === 'agents' && agentsData?.total > 20) || 
              ...
              <div className="flex justify-center gap-2 mt-8">
                ...
                <span className="px-4 py-2 text-gray-700 dark:text-gray-300">{t('pageInfo', { page })}</span>
                <button
