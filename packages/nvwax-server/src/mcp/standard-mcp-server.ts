 ...
 * NvwaX 标准 MCP 适配层
 * ------------------------------------------------------------
 * 背景：nvwax-mcp-server.ts 暴露的 POST /api/mcp/tools/list、POST /api/mcp/tools/call
 *   是【自定义 HTTP JSON 协议】，不是 Model Context Protocol 标准传输。
 *   DeepSeek Harness 的 dsh-mcp-client 是标准 MCP 客户端（stdio / streamable-http
 *   + JSON-RPC），无法直连现有端点。本文件把现有的 MCPToolExecutor（6 个能力工具）
 *   包装成标准 MCP server：
 ...
 *
 * 接入方式见 docs/DSH-MCP-INTEGRATION.md；DSH 侧配置见 examples/dsh/nvwax-mcp.cordis.patch.yml。
 */
